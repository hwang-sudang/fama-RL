...
# move to /home/ubuntu2010/바탕화면/DEV/trading14/src/actor_pretrain
# base_dir2 = '/nas3/hwang/pretrains/pretrain_supervised'
base_dir = '/home1/hwang/trading14'
...
PRETRAIN_DIR = base_dir # base_dir2
...
def load_data(args):
    ...
    for tic in ticker_list:
        ...
        # y_data[y_data.index<=TEST_END]
        dataset = dataset[dataset.index>=args.initial_date]
        dataset = dataset[dataset.index<=args.final_date]
...
def train(a_name, model, dataloader, num_epochs:int, device, args):
    '''
    Args
    ...
    train_loader, val_loader, test_loader = dataloader
    input_size = train_loader.dataset.input_size() # 이게 어떻게 나오는거지
...
def main(args):
    ...
    checkpoint_directory = create_directory_tree(mode=args.mode,
                                                ...
                                                checkpoint_directory=PRETRAIN_SAVEDIR)
    print("\n*----------------------------------------------------------------*\n")
    ...
    for a_name in asset_datasets.keys():
        ...
        asset_result = train(a_name, model, dataloader, args.n_epochs, device, args)
        whole_test_result[f'{a_name}'] = {"MAE":asset_result[0], "RMSE":asset_result[1],
                                          "MAPE":asset_result[2], "R2":asset_result[3]}
        ...
        try:
          ...
          whole_df= whole_df.rename(columns=whole_df.iloc[0]).drop(whole_df.index[0])    
          whole_df = whole_df[["MAE", "RMSE", "MAPE", "R2"]]
        except:
            pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('--layer_size',  type=int,   default=64,      help='Number of neurons in the various hidden layers')
    ...
    parser.add_argument('--initial_date',      type=str,   default='2004-09-30',  help="Initial date of the multidimensional time series of the assets price: str, larger or equal to '2019-07-03'")
    parser.add_argument('--final_date',        type=str,   default='2022-12-31',  help="Final date of the multidimensional time series of the assets price: str, smaller or equal to '2020-12-30'")    
    ...
    parser.add_argument('--gpu_devices',          type=int,            default=6,            help='Specify the GPUs if any')
    ...
    TEST_END = args.final_date # 20221231
    ...
    elif '5' in args.country:
        STOCK_SPACE_DIMENSION = 5
    elif '10' in args.country:
        STOCK_SPACE_DIMENSION = 10
    else:
