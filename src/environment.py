...
class Environment(gym.Env):
    ...
    def __init__(self,
        ...
        self.famafrench = FamaFrench('2000-01-03', '2021-12-31') 

        # 금융 시계열 데이터와 관련된 속성들
        self.stock_market_history = stock_market_history
        self.assets_list = self.stock_market_history.ticker.unique().tolist() ### 컬럼에 따라 자산을 정한다?
        self.stock_space_dimension = len(self.assets_list) #####

        # close 가격만 모여있는 df 필요함
        self.close = pd.DataFrame()

        for a in self.assets_list:
            temp = stock_market_history[stock_market_history['ticker']==a]['close']
            self.close[a] = temp
            # self.close = pd.concat([self.close, temp], axis=1)

            
        
        # 옵션에 따라 추가: sliding correlation matrix of the time-series
        if use_corr_matrix :
            self.stock_market_history = append_corr_matrix(df=self.close,
                                                            window = window)
        elif use_corr_eigenvalues :
            self.stock_market_history = append_corr_matrix_eigenvalues(df=self.close,
                                                                        window=window,
    ...
    def step(self,
        ...
        if self.agent_type == 'default' or "whole":
            ...
            reward = self.famafrench.calcFactorReturn(factor, actions, agent="SMB")
        elif self.agent_type == 'HML':
            reward = self.famafrench.calcFactorReturn(factor, actions, agent="HML")
        elif self.agent_type == 'premium':
            reward = self.famafrench.calcFactorReturn(factor, actions, agent="premium")     
        else :
    ...
    def _buy(self, idx: int, action: int = 1, ) -> None:
        ...
        # 여기서 WARNING 발생 중 : RuntimeWarning: invalid value encountered in double_scalars
